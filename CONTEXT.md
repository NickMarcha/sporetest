...
With the canvas focused, W/S request forward/reverse travel and A/D strafe relative to the current facing. Diagonal input is normalized. Strafing preserves facing. Hold right mouse and drag horizontally to change facing, including while stopped. Left-drag still orbits the camera. Mouse steering uses 0.004 radians per horizontal CSS pixel. Facing changes immediately; the velocity ramp carries the old ground velocity into the new facing frame, so translation remains continuous. Planted targets remain fixed, airborne landings redirect, and stationary turns schedule settling steps. Large turns can exceed leg reach. Pointer release, cancellation, focus loss, pause, and leaving Walk end mouse steering. Player velocity changes linearly with bounded acceleration of 12 baseline cruising speeds per second and braking of 20. At the default multiplier, reaching full speed takes 83 ms, stopping takes 50 ms, a right-angle redirect takes 118 ms, and a full reversal takes 167 ms. These durations depend on velocity change, independently of the stepping cycle. Analytic integration preserves root position and velocity when commands interrupt ramps.
...
Player direction changes can advance a recovery step. At each changed movement command, the planner compares planted contacts with their neutral positions predicted 80 ms ahead, divided by gait tempo. A deviation above 32% of leg length can advance one leg group, led by the most displaced eligible foot. Existing flights finish first, a 40 ms preparation delay applies, and each foot gets at least 15% of a cycle planted before lifting again. These delays also scale with tempo. A group can advance independently only when foot count times duty factor is at least one, so its evenly spaced planted intervals cover the whole cycle. Groups that rely on another group for support keep their existing rhythm. Recovery preserves the group's relative triggers and never postpones an earlier pending step, so repeated mouse events cannot starve recovery. The compiled command records the adjusted group origin, keeping backward scrubbing and skipped frames deterministic. This response runs on changed player intent, not every rendered frame; it does not guarantee reach or dynamic support for every creature, duty factor, or abrupt turn.
