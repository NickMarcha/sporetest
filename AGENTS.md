...
The editor meshes an authored spine and recursive limb chains, derives a branching rig, and binds the skin from field provenance with adjacency smoothing. Sockets carry attachments with their parent source. Bend, limb-flex, and two-phase IK previews exercise the binding without changing the creature. IK exposes head, tail, foot, and grasper targets. The head drives the root, reduced spine particles respond to tail and limb goals, quintic curves reconstruct the spine, and limbs solve against that pose. Stand measures skin contact with the floor and offers bounded torso correction toward measured support while preserving sole contact. Walk adds straight and curved stepping with leg groups, duty factors, measured planted-contact correction, variable speed, turn-rate steering, planted-foot heading control, smooth starts and stops, sequential settling into the standing stance, and optional anticipatory torso weight transfer and step-driven body bob and tilt with contact-driven travel reduction, plus delayed attached-tail follow-through, acceleration lean, inertial tail sway, and positional head stabilization. Stand and Walk expose a static support diagnostic using a rest-surface mass estimate and measured planted-foot contact areas. Walk around adds forward/reverse travel, A/D strafing, right-mouse facing control, normalized diagonals, a threefold travel and stepping tempo, responsive acceleration and braking, continuous airborne landing correction, command-triggered early recovery steps, and a following camera over fixed ground. The gait playground exposes duty factor, cycle duration, foot lift, pause, slow motion, frame stepping, time scrubbing, and sole targets. Preview settings restore from URL search parameters. Recipes can be saved and loaded as JSON, and the current recipe is saved locally in the browser and restored on refresh. Separate attached meshes remain unimplemented.
...
All directories below exist. `rig/` implements spine and limb binding; `anim/` implements pose evaluation, bend diagnostics, both phases of Particle IK, static standing, and a first walking preview. Constraint tuning, dynamic balance, full orientation goals and secondary motion beyond tails remain planned.
